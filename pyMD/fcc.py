...
def FCC(n,offset=[0,0,0],a=1,verbose=False):
    ...
    if verbose:
        if Log.current_logger:
            pr = Log.current_logger.print
        else:
            pr = print 
        pr('FCC:')
        pr('  number of points generated:'+str(n))
        pr('  lattice constant a        :'+str(a))
        pr('  offset                    :'+str(offset))
        pr('  maximum number of cells in each direction:'+str(nb))
        pr('  x :'+str(x))
        pr('  y :'+str(y))
        pr('  z :'+str(z))
        pr('(call fcc with verbose=False to suppress this output)')
    return x,y,z,nb
