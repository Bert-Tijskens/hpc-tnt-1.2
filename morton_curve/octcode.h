...
//===============================================================================
// original code  in ~/Documentation?SpaceFillingCurves/vo-silva-scheidegger-pascucci/sfc
...
{
public:
  OctCode(float const min[3], float const max[3]);
  void setBoundingBox(float const min[3], float const max[3]);
  OCTCODE zOrder (float const* p, int max_level=MAX_LEVEL);
  OCTCODE hilbert(float const* p, int max_level=MAX_LEVEL);
private:
  float bbMin[3];
  float bbMax[3];
};

inline OctCode::OctCode(float const min[3], float const max[3])
{
  this->setBoundingBox(min, max);
}

inline void OctCode::setBoundingBox(float const min[3], float const max[3])
{
  memcpy(this->bbMin, min, sizeof(this->bbMin));
  memcpy(this->bbMax, max, sizeof(this->bbMax));
}

inline OCTCODE OctCode::zOrder(float const* p, int max_level)
{
  /*
	000   000
	100   001
...
inline OCTCODE OctCode::hilbert(float const* p, int max_level)
{
  /*
	000   000
	100   001
